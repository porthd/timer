 ...
 *
 */
class BetterMappingProcessor implements DataProcessorInterface
{
    ...
    protected const DEFAULT_OUTPUTFIELD = 'holidayListJson';
    /**
    ...
    public function process(
        ...
        // get parameter for input and output and check, if the values are allowed
        ...
        if (array_key_exists(self::ATTR_FLEX_YAMLSTARTKEY, $processorConfiguration)) {
            $yamlStartKey = $cObj->stdWrapValue(self::ATTR_FLEX_YAMLSTARTKEY, $processorConfiguration, false);
        } else {
            $yamlStartKey = '';
        }
                ...
                switch ($type) {
                    case self::VAL_LEAF_TYPE_DATETIME:
                        ...
                        if ($help instanceof DateTimeInterface) {
                            $inValue = $help->format($dateFormat);
                        } else {
                            throw new TimerException(
        ...
        switch ($outputFormat) {
            ...
            case self::VAL_OUTPUT_FORMAT_YAML:
                $processedData[$outputFieldName] = CsvYamlJsonMapperUtility::mapAssoativeArrayToYaml(
                    $result,
                    $yamlStartKey
                );
                break;
